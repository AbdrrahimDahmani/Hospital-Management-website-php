...
<div id="new-report" class="container page">
			...
			?>
				...
				<table class="table table-striped link-patients">
					<tbody>
						<tr>
							<td>Patient Name:</td>
							<td><a href="../patients/profile.php?id=<?php echo patient_info("id", $id); ?>" class="patient"><?php echo patient_info("name", $id); ?></a></td>
						</tr>
						<tr>
							<td>Patient ID:</td>
						...
						<tr>
							<td>Registered By:</td>
							<td><a class="staff" href="../staff/profile<?php echo $extension; ?>?id=<?php echo patient_info("physician", $id); ?>"><?php echo staff_info("full_name", patient_info("physician", $id)); ?></a></td>
						</tr>
						<tr>
							<td>Last Update:</td>
				...
				<form method="post" action="" enctype="multipart/form-data">
					<div class="form-group"><label>Token Charge Mode:</label><select class="form-control" name='charge' class="inputOne" id='charge' size='1'>
							...
							<option value="d" selected="selected">Charge Mode D - <?php echo "$global_permission->currency" ?> <?php echo charge_mode("d"); ?></option>
						</select></div>
	...
	-->
					<div class="form-group"><label>Assign to:</label><select class="form-control" name='engaged_by' id='engaged_by' size='1' tabindex='1'>
							...
							$sql = mysqli_query($con, "select * from p_staff_dir where branch=$my_branch and status='active' order by first_name desc limit 2000") or die(mysqli_error($con));

		...
		*/
			...
			<table class="table table-striped link-patients link-patients">
				<tbody>
					...
					<tr>
						<td>Token Charge:</td>
						<td><?php echo charge_mode(report_info("charge", $id)); ?> <?php echo "$global_permission->currency" ?></td>
					</tr>
					<?php if (report_info("checkout_charges", $id) != "") {  ?>
						<tr>
							<td>Medicine Charge:</td>
							<td class="checkout"><?php echo report_info("checkout_charges", $id); ?> <?php echo "$global_permission->currency" ?> (Token charges not included!)</td>
						</tr>
					<?php } else { ?>
						<tr>
							<td>Medicine Charge:</td>
					...
					<?php if (report_attachment("exist", $id) == true) { ?><tr>
							<td>Reports:</td>
							<td><a href="../media/reports/<?php echo "$id" ?>.zip">Download attachments</a> </li>
								<li>
							</td>
						</tr><?php } ?>
					<tr>
					...
					<?php if (report_info("reply", $id) != "") {; ?> <tr>
							<td>Reply:</td>
							<td><textarea readonly class="form-textarea"><?php echo report_info("reply", $id); ?></textarea></td>
						</tr><?php } ?>
					<?php
					...
					?>
						<tr>
							<td>- <?php echo $medicines['medicine'] ?></td>
							<td><?php echo $medicines['doses'] ?> (<?php echo $medicines['timings'] ?>) for <?php echo $medicines['days'] ?> day(s)</td>
						</tr>
					<?php } ?>
				</tbody>
			</table>
				...
				<div class="panel-body">
					...
					<table class="table table-striped link-patients  link-patients">
						<tbody>
							<tr>
								<td>Patient:</td>
								<td><a href="../patients/profile.php?id=<?php echo $patient_id; ?>" class="patient"><?php echo patient_info("name", $patient_id); ?></a> | <?php echo patient_info("serial", $patient_id); ?>-<?php echo patient_info("id", $patient_id); ?></td>
							</tr>
							<tr>
								<td>Registered At:</td>

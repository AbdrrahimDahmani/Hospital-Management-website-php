		...
		<div class="panel-body">
			...
			<table class="table table-striped link-patients link-patients">
				<tbody>
					...
					<tr>
						<td>Token Charge:</td>
						<td><?php echo "$global_permission->currency" ?> <?php echo charge_mode(report_info("charge", $id)); ?></td>
					</tr>

					<?php if (report_info("checkout_charges", $id) != "") {  ?>
						<tr>
							<td>Medicine Charges:</td>
							<td><?php echo "$global_permission->currency" ?> <?php echo report_info("checkout_charges", $id); ?></td>
						</tr>
					<?php } else { ?>
						<tr>
							<td>Medicine Charges:</td>
					...
					<tr>
						<td>Symptoms:</td>
						<td><textarea readonly class="form-textarea"><?php echo report_info("symptoms", $id); ?></textarea></td>
					</tr>
					<?php
					$sql = mysqli_query($con, "select * from p_med_record where report_id='$id' order by last_update desc limit 9000") or die(mysqli_error($con));
					while ($list_med = mysqli_fetch_array($sql)) {
					?>
						<tr>
							<td>- <?php echo $list_med['medicine'] ?></td>
							<td><?php echo $list_med['doses'] ?> (<?php echo $list_med['timings'] ?>) for <?php echo $list_med['days'] ?> day(s)</td>
						</tr>
					<?php } ?>
				</tbody>
			</table>
			...
			<form method="post" action="" enctype="multipart/form-data">
				<div class="form-group"><label>Medicine:</label><select class="form-control" name='medicine' id='medicine' size='1' tabindex='1'>
						<?php
						$sql = mysqli_query($con, "select * from p_stock where branch='$get_composer_branch' order by code asc limit 9000") or die(mysqli_error($con));
						while ($medicines = mysqli_fetch_array($sql)) {
						?>
							<option value='<?php echo $medicines['code'] ?>'><?php echo $medicines['code'] ?> <?php if (display_permission("medicine_profile") == true) {
				...
				<div class="panel-body">
					<table class="table table-striped link-patients link-patients">
						<tbody>
							<tr>
								<td>Patient:</td>
								<td><a href="../patients/profile.php?id=<?php echo patient_info("id", $patient_id); ?>" class="patient"><?php echo patient_info("name", $patient_id); ?></a> | <?php echo patient_info("serial", $patient_id); ?>-<?php echo patient_info("id", $patient_id); ?></td>
							</tr>
							<tr>
								<td>Registered:</td>
								<td><?php echo $global_permission->guardian_short_name;
									echo patient_info("branch", $patient_id); ?> - <?php echo branch_name(patient_info("branch", $patient_id)); ?></td>
							</tr>
							<tr>
								<td>Registered By:</td>
								<td><a class="staff" href="../staff/profile.php?id=<?php echo patient_info("physician", $patient_id); ?>"><?php echo staff_info("full_name", patient_info("physician", $patient_id)); ?></a></td>
							</tr>
							<tr>
								<td>Last Update:</td>
								<td><?php echo display_time(patient_info("last_update", $patient_id)); ?></td>
							</tr>
						</tbody>
					</table>
				</div>

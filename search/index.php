			...
			<?php foreach ($trimmed_array as $trimm) { ?>
				...
				<?php if (display_permission("staff_directory") == true && $query != "") {; ?>
					<div class="row">
						...
						<?php
						$sql = mysqli_query($con, "select * from p_staff_dir where first_name like '%$trimm%' or last_name like '%$trimm%' or full_name like '%$trimm%' or id like '%$trimm%' limit 1000") or die(mysqli_error($con));
								...
								<div class="panel panel-default profile-card profile-branch">
									...
									<div class="panel-body">
										...
										<p><i class="glyphicon glyphicon-user"></i><span class="small_id_icon">ID#
												<?php echo $staff['id'] ?> — </span><span class="small_level_icon"><?php echo "$global_permission->guardian_short_name";
										...
										<p><i class="glyphicon glyphicon-facetime-video"></i>SKYPE: <?php echo $staff['skype'] ?>
										</p>
									</div>
								</div>
							</a>
				...
				<?php if (display_permission("patients_directory") == true && $query != "") {; ?>
					<div class="row">
						<ul class="list-group row ul-patients">
							<?php
							$sql = mysqli_query($con, "select * from p_patients_dir where name like '$query' or serial like '%$trimm%' or friendly_name like '%$trimm%' or id like '%$trimm%' or contact like '%$trimm%' or email like '%$trimm%' limit 1000") or die(mysqli_error($con));
				...
				<?php if (display_permission("medicine_profile") == true && $query != "") { ?>
					<div class="row">
						...
						<?php
						$sql = mysqli_query($con, "select * from p_medicine_dir where code like '%$trimm%' or name like '%$trimm%' or id like '%$trimm%' limit 1000");
						...
						?>
							<div class="panel panel-default profile-card profile-medicines">
								...
								<div class="panel-body">
									<strong><?php echo $medicines['category'] ?></strong> | <?php echo $medicines['price'] ?>
				...
				<div class="row">
					<table class="table table-striped link-patients link-patients">
						<tbody>
							<?php
							if ($query != "") {
								$sql = mysqli_query($con, "select * from p_reports where id like '%$trimm%' or symptoms like '%$trimm%'  order by last_update desc limit 100") or die(mysqli_error($con));

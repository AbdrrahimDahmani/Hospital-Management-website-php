	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<table class="table table-striped">
				<tbody>
					<?php
					$sql = mysqli_query($con, "select * from p_logs where user='$id' order by id desc limit 500") or die(mysqli_error($con));
					while ($display_log = mysqli_fetch_array($sql)) {
					?>
						<tr>
							<td><?php echo display_time($display_log['at']); ?><span class="priority_<?php echo priority_level($display_log['priority']); ?>">: <?php echo $display_log['action'] ?></span></td>
						</tr>
					<?php } ?>
				</tbody>
			</table>

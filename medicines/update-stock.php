		...
		<div class="panel-body">
			...
			<form method="post" action="" enctype="multipart/form-data">
				<div class="form-group"><label>Branch:</label><select class="form-control" name='branch' id='branch' size='1' tabindex='1'>
						...
						?>
							<option value='<?php echo $clinics['id'] ?>'><?php echo "$global_permission->guardian_short_name";
																		echo $clinics['id'] ?> - <?php echo $clinics['name'] ?></option>
						<?php } ?>
					</select></div>

				<div class="form-group"><label>Medicine:</label><select class="form-control" name='code' id='code' size='1' tabindex='1'>
						<?php
						$sql = mysqli_query($con, "select * from p_medicine_dir order by last_update desc limit 9000") or die(mysqli_error($con));

...
function count_lstaff($branch,$hours = NULL){
	...
	if(!isset($hours)) $hours = 24; 
		...
		$total = 0;
		$sql=mysqli_query($con, "select * from p_logs where action='LoggedIn' and `at` > NOW() - INTERVAL $hours HOUR") or die(mysqli_error($con));
		$user_data = array();
...
function staff_delete($id){
	...
	mysqli_query($con, "delete from p_staff_dir where id='$id'") or die(mysqli_error($con));

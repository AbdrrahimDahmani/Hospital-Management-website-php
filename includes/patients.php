...
function delete_single_patient($id)
{
	...
	mysqli_query($con, "delete from p_patients_dir where id='$id'") or die(mysqli_error($con));
	mysqli_query($con, "delete from p_reports where patient='$id'") or die(mysqli_error($con));
...
function count_today_sales($branch, $hours = NULL)
{
	...
	$sql = mysqli_query($con, "select * from p_reports where last_update > NOW() - INTERVAL $hours HOUR and branch='$branch' and signed_by!='' ") or die(mysqli_error($con));

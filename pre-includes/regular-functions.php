...
function display_time($datetime)
{
	...
	if ($today == $extract_day) {
		$output = "" . date("g:i a", $processing) . " Today";
	} else {
		$output = date("F j, Y, g:i a", $processing);
	}
...
function access_level2rank($access_level)
{
	...
	if ($access_level == 6) {
		$rank = "$global_permission->access_level_6";
	} else if ($access_level == 5) {
		$rank = "$global_permission->access_level_5";
	} else if ($access_level == 4) {
		$rank = "$global_permission->access_level_4";
	} else if ($access_level == 3) {
		$rank = "$global_permission->access_level_3";
	} else if ($access_level == 2) {
		$rank = "$global_permission->access_level_2";
	} else {
		$rank = "$global_permission->access_level_1";
	}
...
function charge_mode($mode)
{
	global $global_permission;
	if ($mode == "a") {
		$charge = "$global_permission->charge_mode_a";
	} else if ($mode == "b") {
		$charge = "$global_permission->charge_mode_b";
	} else if ($mode == "c") {
		$charge = "$global_permission->charge_mode_c";
	} else {
		$charge = "$global_permission->charge_mode_d";
	}
	return $charge;
}
...
function page_permission($pagename)
{
	...
	$sql = mysqli_query($con, "select * from p_global_permissions");
	while ($permission_for = mysqli_fetch_array($sql)) {
		if ($access_level >= $permission_for[$pagename]) {
			return true;
		} else {
			print "<script>";
...
function display_permission($query, $user = NULL)
{
	...
	$sql = mysqli_query($con, "select * from p_global_permissions");
	while ($permission_for = mysqli_fetch_array($sql)) {
		if ($access_level >= $permission_for[$query]) {
			return true;
		} else {
			return false;
		}
